...
<div class="admin-shell">
    ...
    <div class="admin-main">
        <div class="admin-topbar">
            <div class="topbar-titles"><div class="crumb">{{ $crumb ?? '' }}</div><h1>@yield('title', 'Dashboard')</h1></div>
            <div class="topbar-actions">
                ...
                <a href="{{ $profileRoute }}" class="user-chip"><div class="avatar" style="overflow:hidden;flex:0 0 30px;">@if(auth()->user()->profile_image_url)<img src="{{ auth()->user()->profile_image_url }}" alt="{{ auth()->user()->name }}" style="display:block;width:30px;height:30px;max-width:30px;object-fit:cover;border-radius:50%;">@else{{ strtoupper(substr(auth()->user()->name ?? 'U', 0, 1)) }}@endif</div><div class="meta"><strong>{{ auth()->user()->name ?? 'User' }}</strong><span>Profile</span></div></a>
