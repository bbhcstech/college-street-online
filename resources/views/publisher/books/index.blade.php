...
@section('nav')
    ...
    <div class="nav-group">
        <div class="nav-group-title">Catalogue</div><a href="{{ route('publisher.books.index') }}"
            class="nav-link active"><span class="nav-icon">📖</span><span>My Books</span></a><a
            href="{{ route('publisher.inventory.index') }}" class="nav-link"><span
                class="nav-icon">📦</span><span>Inventory</span></a>
    </div>
    <div class="nav-group">
...
@section('content')
    <div class="publisher-page-head">
        ...
        </div><a href="{{ route('publisher.books.create') }}" class="btn btn-primary">+ Add book</a>
    </div>
    ...
    <div class="a-card publisher-table-card" data-publisher-books
        ...
        <form method="GET" class="publisher-book-toolbar">
            <div class="publisher-search"><span>⌕</span><input name="q" value="{{ request('q') }}"
                    placeholder="Search title, ISBN or author"></div><select name="category_id" class="a-select">
            ...
            href="{{ route('publisher.books.index') }}" class="btn btn-outline btn-sm">Reset</a>@endif
        </form>
                ...
                <tbody>
                    @forelse($books as $book)
                        <tr data-export-row data-id="{{ $book->id }}">
                            ...
                            <td>
                                ...
                                alt="{{ $book->title }} cover" class="a-book-cover-thumb">@else<div
                                        class="a-book-cover-thumb a-book-cover-placeholder">📖</div>@endif<div><strong
                                            data-cell>{{ $book->title }}</strong><small
                                            data-cell>{{ $book->author?->name ?? 'Unknown author' }}</small></div>
                                </div>
                            </td>
                            ...
                            <td data-cell><span
                                    class="publisher-stock {{ ($book->inventory?->quantity ?? 0) <= ($book->inventory?->low_stock_threshold ?? 5) ? 'low' : '' }}">{{ $book->inventory?->quantity ?? 0 }}</span>
                            </td>
                            <td>
                                <form method="POST" action="{{ route('publisher.books.status', $book) }}">@csrf
                                    @method('PATCH')<select name="status" class="book-status-select status-{{ $book->status }}"
                                        data-current="{{ $book->status }}"
                                        onchange="if(confirm('Change this book status?'))this.form.submit();else this.value=this.dataset.current">
                            ...
                            <td>
                                <div class="book-row-actions"><a href="{{ route('books.show', $book) }}" target="_blank"
                                        class="btn btn-outline btn-sm">View</a><a
                                        href="{{ route('publisher.books.edit', $book) }}" class="btn btn-outline btn-sm">Edit</a>
                                    <form method="POST" action="{{ route('publisher.books.destroy', $book) }}"
                                        ...
                                        @method('DELETE')<button class="btn btn-danger-outline btn-sm">Archive</button></form>
                                </div>
                            </td>
                        </tr>
                    @empty<tr>
        ...
        <div class="publisher-table-footer"><span>Showing {{ $books->firstItem() ?? 0 }}–{{ $books->lastItem() ?? 0 }} of
                {{ $books->total() }} books</span>@if($books->hasPages())
                    <nav class="order-pagination">@if($books->onFirstPage())<span class="disabled">Previous</span>@else<a
                    href="{{ $books->previousPageUrl() }}">Previous</a>@endif
                        @foreach(range(1, $books->lastPage()) as $page)<a href="{{ $books->url($page) }}"
