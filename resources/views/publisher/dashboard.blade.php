...
@section('content')
    ...
    <div class="publisher-dashboard-stats">
        <div class="stat-box">
            <div class="stat-top">
                ...
                <span class="trend-chip trend-up">This month</span>
            </div>
        ...
        <div class="stat-box">
            <div class="stat-top">
                ...
                <span class="trend-chip trend-up">{{ $monthlyOrders }}</span>
            </div>
        ...
        <div class="stat-box">
            <div class="stat-top">
                ...
                <span class="trend-chip trend-up">{{ $activeBookCount }} active</span>
            </div>
        ...
        <div class="stat-box">
            <div class="stat-top">
                ...
                <span class="trend-chip {{ $pendingFulfillment ? 'trend-down' : 'trend-up' }}">Action</span>
            </div>
    ...
    <div class="publisher-dashboard-health">
        ...
        <div class="{{ $lowStockCount ? 'attention' : '' }}"><strong>{{ $lowStockCount }}</strong>
    ...
    <div class="dashboard-chart-grid">
        <section class="a-card dashboard-chart-card">
            <div class="a-card-head dashboard-card-title">
                ...
                <div class="chart-legend">
                    <span><i class="legend-orders"></i>Units</span><span><i class="legend-revenue"></i>Revenue</span></div>
            </div>
            <div class="weekly-chart">
                @foreach($weeklyTrend as $point)
                <div class="weekly-column" title="{{ $point['units'] }} units · ₹{{ number_format($point['revenue'], 0) }}">
                    <div class="bar-area">
                        <i class="revenue-bar" style="height:{{ $point['revenue'] > 0 ? max(4, $point['revenue'] / $maxRevenue * 100) : 0 }}%"></i>
                        <i class="order-bar" style="height:{{ $point['units'] > 0 ? max(4, $point['units'] / $maxUnits * 100) : 0 }}%"></i>
                    </div>
        ...
        <section class="a-card">
            ...
            <div class="status-chart-wrap">
                <div class="status-donut" style="background:conic-gradient({{ implode(',', $gradient) ?: '#DCE2ED 0 100%' }});">
                    <div>
                        <strong>{{ $statusMix->sum() }}</strong>
                        <span>Orders</span>
                    </div>
                </div>
                <div class="status-legend">
                    @forelse($statusMix as $status => $count)
                        <div>
                            <i style="background:{{ $statusColors[$status] ?? '#8A96A8' }}"></i>
                            <span>
