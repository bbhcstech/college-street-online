...
<body>
    ...
    <table>
        ...
        <tbody>
            @foreach($books as $book)@php($quantity = $book->inventory?->quantity ?? 0)@php($threshold = $book->inventory?->low_stock_threshold ?? 5)
            <tr>
                <td>{{ $book->title }}</td>
                <td>{{ $book->isbn }}</td>
                <td>{{ $quantity }}</td>
                <td>{{ $threshold }}</td>
                <td>{{ $quantity <= $threshold ? 'Low stock' : 'Healthy' }}</td>
