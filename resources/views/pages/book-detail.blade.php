...
@section('content')
    ...
    <div class="container" style="padding-top:24px;">
        <div class="breadcrumb-row"><a href="{{ route('home') }}">Home</a><span class="sep">/</span><a
                href="{{ route('books.index') }}">Browse Books</a><span class="sep">/</span><span
                class="current">{{ $book->title }}</span></div>
    </div>
            ...
            <div class="product-layout">
                <div class="product-visual reveal in-view">
                    <div class="product-cover-frame">
                        @if($book->cover_url)
                            <div class="book-cover book-cover-uploaded"><img src="{{ $book->cover_url }}"
                                    alt="{{ $book->title }} cover" class="book-cover-image"></div>
                        @else
                ...
                <div class="product-information reveal in-view">
                    <div class="product-heading-row"><span
                            class="badge-tag-outline">{{ $book->category->name ?? 'General' }}</span>@if($discount)<span
                            class="product-discount">{{ $discount }}% OFF</span>@endif</div>
                    <h1>{{ $book->title }}</h1>
                    <p class="product-byline">by
                        <strong>{{ $book->author->name ?? 'Unknown author' }}</strong><span></span>Published by
                        <strong>{{ $book->publisher->business_name ?? 'Independent publisher' }}</strong></p>

                    <div class="product-rating-row">
                        <span class="rating-chip">{{ $averageRating ? number_format($averageRating, 1) : 'New' }}
                            @if($averageRating)&#9733;@endif</span>
                    ...
                    <div class="product-purchase-box">
                        <form method="POST" action="{{ route('cart.store') }}" class="product-cart-form">
                            ...
                            <input id="book-quantity" type="number" name="quantity" value="1" min="1" @if($stock > 0)
                            max="{{ $stock }}" @endif class="form-control" @disabled($stock <= 0)>
                            <button type="submit" class="btn btn-primary" @disabled($stock <= 0)>{{ $stock > 0 ? 'Add to Cart' : 'Out of Stock' }}</button>
                        </form>
                        @guest<p class="purchase-login-note">You can view every detail as a guest. <a
                        href="{{ route('account.login') }}">Log in to purchase</a>.</p>@endguest
                    </div>
    ...
    <section class="section section-alt" id="reader-reviews">
        <div class="container" style="max-width:900px;">
            <div class="section-head">
                <div><span class="eyebrow"><span class="dot"></span> Reader Feedback</span>
                    <h2>Reviews &amp; Ratings</h2>
                </div>
                <div class="review-average">
                    <strong>{!! $averageRating ? number_format($averageRating, 1) : '&mdash;' !!}</strong><span
                        class="review-stars">&#9733;&#9733;&#9733;&#9733;&#9733;</span><small>{{ $reviewCount }}
                        {{ Str::plural('review', $reviewCount) }}</small></div>
            </div>
                ...
                @forelse($book->reviews->sortByDesc('created_at') as $review)
                    <article class="card review-card">
                        <div class="review-card-head">
                            <div><strong>{{ $review->customer->name }}</strong><small>Verified purchase</small></div><span
                                class="review-stars">{!! str_repeat('&#9733;', $review->rating) !!}{!! str_repeat('&#9734;', 5 - $review->rating) !!}</span>
                        </div>
                        @if($review->review)
        ...
        <section class="section">
            <div class="container">
                <div class="section-head reveal">
                    <div><span class="eyebrow"><span class="dot"></span> You May Also Like</span>
                        <h2>Related Titles</h2>
                    </div><a href="{{ route('books.index', ['category' => $book->category?->slug]) }}">View category &rarr;</a>
                </div>
                <div class="grid grid-4">@foreach($related as $r) @include('partials.book-card', ['book' => $r]) @endforeach
                </div>
            </div>
        </section>
    @endif
