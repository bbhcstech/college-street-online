<footer class="site-footer">
    <div class="container">
        <div class="footer-cta reveal in-view">
            <span class="eyebrow" style="background:rgba(255,255,255,0.08);color:var(--accent-gold-light);border-color:rgba(237,161,58,0.3);">Stay in the Loop</span>
            <h2>Never miss a <em>new arrival</em></h2>
            <p style="color:#B8C4D6;max-width:480px;margin:0 auto;">Subscribe for new releases, bestseller drops, and exclusive coupons.</p>
            <form method="POST" action="{{ route('newsletter.subscribe') }}" style="display:flex;gap:10px;max-width:420px;margin:20px auto 0;flex-wrap:wrap;justify-content:center;">
                @csrf
                <input type="email" name="email" required placeholder="you@email.com" class="form-control" style="flex:1;min-width:220px;">
                <button type="submit" class="btn btn-gold">Subscribe</button>
            </form>
        </div>
        <div class="footer-grid">
            <div>
                <a href="{{ route('home') }}" class="brand" style="color:#fff;margin-bottom:16px;"><img src="{{ asset('images/logo-square.jpg') }}" alt="College Street Online" style="height:40px;border-radius:8px;"></a>
                <p style="color:#9AA9C2;max-width:320px;margin-top:16px;">College Street Online brings Kolkata's legendary book market to your doorstep &mdash; academic titles, Bengali literature, and everything in between, from publishers you trust.</p>
            </div>
            <div><h4>Shop</h4><ul style="display:flex;flex-direction:column;gap:10px;">
