                        ...
                        <tr>
                            <td colspan="10" style="padding: 40px 20px; text-align: center;">
                                <div class="analytics-empty order-empty">
                                    <strong style="font-size: 1rem; color: var(--a-text); display: block; margin-bottom: 4px;">No orders found</strong>
                                    <span style="font-size: 0.82rem; color: var(--a-text-muted);">Try changing or resetting your active search or filters.</span>
                                </div>
                            </td>
                        </tr>
                    @endforelse
            ...
            @if($orders->hasPages())
                <div class="order-pagination" style="display: flex; gap: 6px; align-items: center;">
                    ...
                    @foreach ($orders->getUrlRange(max(1, $orders->currentPage() - 2), min($orders->lastPage(), $orders->currentPage() + 2)) as $page => $url)
                        ...
                        @else
                            <a href="{{ $url }}" class="btn btn-outline btn-sm" style="padding: 4px 12px; font-size: 0.8rem;">{{ $page }}</a>
                        @endif
                    @endforeach
                    ...
                    @else
                        <span class="btn btn-outline btn-sm disabled" style="opacity: 0.5; pointer-events: none; padding: 4px 12px; font-size: 0.8rem;">Next &raquo;</span>
                    @endif
                </div>
            @endif
        </div>
